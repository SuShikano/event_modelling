...
If one compare this with that of the exponential distribution ($f(t) = \lambda \exp^{-\lambda t}$), the Weibull distribution has an additional parameter $p$. This parameter is called shape parameter, since it determines whether the PDF is increasing or decreasing function:
...
If $p=1$, the Weibull distribution becomes identical with the exponential distribution. That is, the exponential distribution is a special case of the Weibull distribution.

By applying the same procedure, we can obtain the survival function:


 $S(t) = \exp^{-(\lambda t)^p}$

and the hazard function for the Weibull model:

 $h(t) = \lambda p(\lambda t)^{p-1} = p t^{p-1} \lambda^p = p t^{p-1} $
